...
import { MessageCircle, Search, MoreVertical, User, Send } from 'lucide-react';
...
const MessagesPage: React.FC = () => {
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    const messageId = `${selectedConversation}-${Date.now()}`;
    const now = new Date();
    ...
    setTimeout(() => {
      const autoReplyMessages = [
        ...
        'Ótimo! Vou passar essa informação adiante.',
      ...
      const replyId = `${selectedConversation}-reply-${Date.now()}`;
      const replyTimestamp = `${now.getHours().toString().padStart(2, '0')}:${(now.getMinutes() + 1).toString().padStart(2, '0')}`;
